...
def get_chain(): #เช็คจำนวนบล็อคกี่บล็อคเก็บข้อมูลในลักษณะใด
    response={
        "chain":blockchain.chain, #ข้อมูลแต่ละบล็อคมีอะไรบ้าง
        "length":len(blockchain.chain) #นับจำนวนบล็อคที่อยู่ในบล็อคเชน
    }
    return jsonify(response),200 #ส่งข้อมูล client - server

@app.route('/mining',methods=["GET"])
def mining_block(): #การที่จะminingได้ต้องมี Block ล่าสุดก่อน
